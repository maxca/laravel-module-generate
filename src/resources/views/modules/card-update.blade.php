<div class="card">
    ...
    <div class="card-body">
        <div class="form-group">
            ...
            <input type="text" name="name" class="form-control" value="{{$data->name}}" placeholder="input module name">
        </div>
        <div class="form-group">
            <label for=test">Module key</label>
            <input type="text" name="module_key" class="form-control" value="{{$data->key}}"
                   placeholder="input module key">
        </div>
        <div class="form-group">
...
<div class="card align-top" style="margin-top: 20px">
    ...
    <div class="card-body">
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'create','checked' => $data->action->where('name','create')->count() == 1 ? 'checked' : null ,
            'text_name' => 'create_link' ,
            'placeholder' => 'action create link'
        ])
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'update','checked' => $data->action->where('name','update')->count() == 1 ? 'checked' : null ,
            'text_name' => 'update_link' ,
            'placeholder' => 'action update link'
        ])
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'delete','checked' => $data->action->where('name','delete')->count() == 1 ? 'checked' : null ,
            'text_name' => 'delete_link' ,
            'placeholder' => 'action delete link'
        ])
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'detail','checked' => $data->action->where('name','detail')->count() == 1 ? 'checked' : null ,
            'text_name' => 'detail_link' ,
            'placeholder' => 'action detail link'
        ])
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'export','checked' => $data->action->where('name','export')->count() == 1 ? 'checked' : null ,
            'text_name' => 'export_link' ,
            'placeholder' => 'action export link'
        ])
        @include('module-generate::modules.component.checkbox-text',[
            'name' => 'search','checked' => $data->action->where('name','search')->count() == 1 ? 'checked' : null ,
    ...
    <div class="card-body" id="column-node">
        @foreach($data->column as $key => $column)
            <div class="form-group add-column">
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <div class="input-group-text">
                            <input type="hidden" name="search_column[{{$key}}]" class="hide-search-column" value="active">
                            <input type="checkbox" class="check-search" checked>
                        </div>
                    </div>
                    <input type="text" name="column_name[{{$key}}]" value="{{$column->name}}" class="form-control" placeholder="name">
                    {!! $selectIcon !!}
                    <input type="text" name="key[{{$key}}]" class="form-control" placeholder="key">
                ...
                <div class="input-group mb-3">
                    {!! $selectInputType !!}
                    <input type="text" name="value[{{$key}}]" value="{{$column->value}}" class="form-control" placeholder="value , comma to split">
                    <input type="text" name="label[{{$key}}]" value="{{$column->label}}" class="form-control" placeholder="label">
                </div>
                <div class="input-group mb-3 rule-row" id="append-rule">
                    {!! $selectRule !!}
                    <input type="text" name="rule_value[{{$key}}][]" data-name="rule" class="form-control rule-name"
                           placeholder="value">
                    <div class="input-group-append">
